...
public:
    bool Set(const libzamerica::PaymentAddress& addr);
    CZCPaymentAddress() {}

    CZCPaymentAddress(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCPaymentAddress(const libzamerica::PaymentAddress& addr) { Set(addr); }

    libzamerica::PaymentAddress Get() const;
};

class CZCSpendingKey : public CBase58Data {
public:
    bool Set(const libzamerica::SpendingKey& addr);
    CZCSpendingKey() {}

    CZCSpendingKey(const std::string& strAddress) { SetString(strAddress.c_str(), 2); }
    CZCSpendingKey(const libzamerica::SpendingKey& addr) { Set(addr); }

    libzamerica::SpendingKey Get() const;
};
