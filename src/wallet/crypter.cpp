...
static bool DecryptSpendingKey(const CKeyingMaterial& vMasterKey,
                               ...
                               libzamerica::SpendingKey& sk)
{
    CKeyingMaterial vchSecret;
    if(!DecryptSecret(vMasterKey, vchCryptedSecret, address.GetHash(), vchSecret))
        return false;

    if (vchSecret.size() != libzamerica::SerializedSpendingKeySize)
        return false;
...
{
    {
        ...
        {
            const libzamerica::PaymentAddress &address = (*skmi).first;
...
bool CCryptoKeyStore::AddSpendingKey(const libzamerica::SpendingKey &sk)
{
    {
...
bool CCryptoKeyStore::AddCryptedSpendingKey(const libzamerica::PaymentAddress &address,
...
bool CCryptoKeyStore::GetSpendingKey(const libzamerica::PaymentAddress &address, libzamerica::SpendingKey &skOut) const
{
    {
        LOCK(cs_SpendingKeyStore);
        if (!IsCrypted())
...
{
    {
        ...
        {
            const libzamerica::SpendingKey &sk = mSpendingKey.second;
            CSecureDataStream ss(SER_NETWORK, PROTOCOL_VERSION);
            ss << sk;
            CKeyingMaterial vchSecret(ss.begin(), ss.end());
            libzamerica::PaymentAddress address = sk.address();
            std::vector<unsigned char> vchCryptedSecret;
