...
bool CWallet::AddZKey(const libzamerica::SpendingKey &key)
{
    AssertLockHeld(cs_wallet); // mapZKeyMetadata
...
bool CWallet::AddCryptedSpendingKey(const libzamerica::PaymentAddress &address,
                                    const libzamerica::ViewingKey &vk,
...
bool CWallet::LoadCryptedZKey(const libzamerica::PaymentAddress &addr, const libzamerica::ViewingKey &vk, const std::vector<unsigned char> &vchCryptedSecret)
{
    return CCryptoKeyStore::AddCryptedSpendingKey(addr, vk, vchCryptedSecret);
}

bool CWallet::LoadZKey(const libzamerica::SpendingKey &key)
{
            ...
            for (mapNoteData_t::value_type& item : wtxItem.second.mapNoteData) {
                if (!item.second.nullifier) {
                    ...
                    auto hSig = wtxItem.second.vjoinsplit[i].h_sig(
                        *pzamericaParams, wtxItem.second.joinSplitPubKey);
...
boost::optional<uint256> CWallet::GetNoteNullifier(const JSDescription& jsdesc,
                                                   const libzamerica::PaymentAddress& address,
...
mapNoteData_t CWallet::FindMyNotes(const CTransaction& tx) const
{
    ...
    for (size_t i = 0; i < tx.vjoinsplit.size(); i++) {
        auto hSig = tx.vjoinsplit[i].h_sig(*pzamericaParams, tx.joinSplitPubKey);
...
void CWallet::GetFilteredNotes(std::vector<CNotePlaintextEntry> & outEntries, std::string address, int minDepth, bool ignoreSpent)
{
    ...
    libzamerica::PaymentAddress filterPaymentAddress;
    if (address.length() > 0) {
    ...
    for (auto & p : mapWallet) {
        ...
        for (auto & pair : wtx.mapNoteData) {
            ...
            auto hSig = wtx.vjoinsplit[i].h_sig(*pzamericaParams, wtx.joinSplitPubKey);
            try {
