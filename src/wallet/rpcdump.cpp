...
Value importprivkey(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() < 1 || params.size() > 3)
        throw runtime_error(
            "importprivkey \"zamericaprivkey\" ( \"label\" rescan )\n"
            "\nAdds a private key (as returned by dumpprivkey) to your wallet.\n"
            "\nArguments:\n"
            "1. \"zamericaprivkey\"   (string, required) The private key (see dumpprivkey)\n"
...
Value importaddress(const Array& params, bool fHelp)
{
    ...
    if (address.IsValid()) {
        ...
        throw JSONRPCError(RPC_INVALID_ADDRESS_OR_KEY, "Invalid ZAmerica address or script");
    }
...
Value importwallet_impl(const Array& params, bool fHelp, bool fImportZKeys)
{
    ...
    while (file.good()) {
        ...
        // Let's see if the address is a valid ZAmerica spending key
        if (fImportZKeys) {
            try {
                CZCSpendingKey spendingkey(vstr[0]);
                libzamerica::SpendingKey key = spendingkey.Get();
                libzamerica::PaymentAddress addr = key.address();
                ...
                // Not a valid spending key, so carry on and see if it's a ZAmerica style address.
            }
        }
...
Value dumpprivkey(const Array& params, bool fHelp)
{
    ...
    if (fHelp || params.size() != 1)
        throw runtime_error(
            "dumpprivkey \"zamericaaddress\"\n"
            "\nReveals the private key corresponding to 'zamericaaddress'.\n"
            "Then the importprivkey can be used with this output\n"
            "\nArguments:\n"
            "1. \"zamericaaddress\"   (string, required) The zamerica address for the private key\n"
            "\nResult:\n"
...
Value dumpwallet_impl(const Array& params, bool fHelp, bool fDumpZKeys)
{
    ...
    // produce output
    file << strprintf("# Wallet dump created by ZAmerica %s (%s)\n", CLIENT_BUILD, CLIENT_DATE);
