...
#include "zamerica/JoinSplit.hpp"
#include "zamerica/Note.hpp"
#include "zamerica/NoteEncryption.hpp"
...
CWalletTx GetValidReceive(const libzamerica::SpendingKey& sk, CAmount value, bool randomInputs) {
    ...
    boost::array<libzamerica::JSInput, 2> inputs = {
        libzamerica::JSInput(), // dummy input
        libzamerica::JSInput() // dummy input
    };

    boost::array<libzamerica::JSOutput, 2> outputs = {
        libzamerica::JSOutput(sk.address(), value),
        libzamerica::JSOutput(sk.address(), value)
    };

    boost::array<libzamerica::Note, 2> output_notes;
...
libzamerica::Note GetNote(const libzamerica::SpendingKey& sk,
                       const CTransaction& tx, size_t js, size_t n) {
    ZCNoteDecryption decryptor {sk.viewing_key()};
    auto hSig = tx.vjoinsplit[js].h_sig(*params, tx.joinSplitPubKey);
    auto note_pt = libzamerica::NotePlaintext::decrypt(
        decryptor,
...
CWalletTx GetValidSpend(const libzamerica::SpendingKey& sk,
                        const libzamerica::Note& note, CAmount value) {
    CMutableTransaction mtx;
    mtx.vout.resize(2);
    ...
    libzamerica::JSOutput dummyout;
    libzamerica::JSInput dummyin;

    {
        if (note.value > value) {
            libzamerica::SpendingKey dummykey = libzamerica::SpendingKey::random();
            libzamerica::PaymentAddress dummyaddr = dummykey.address();
            dummyout = libzamerica::JSOutput(dummyaddr, note.value - value);
        } else if (note.value < value) {
            libzamerica::SpendingKey dummykey = libzamerica::SpendingKey::random();
            libzamerica::PaymentAddress dummyaddr = dummykey.address();
            libzamerica::Note dummynote(dummyaddr.a_pk, (value - note.value), uint256(), uint256());
            tree.append(dummynote.cm());
            dummyin = libzamerica::JSInput(tree.witness(), dummynote, dummykey);
        }
    }
    ...
    boost::array<libzamerica::JSInput, 2> inputs = {
        libzamerica::JSInput(tree.witness(), note, sk),
        dummyin
    };

    boost::array<libzamerica::JSOutput, 2> outputs = {
        dummyout, // dummy output
        libzamerica::JSOutput() // dummy output
    };
...
TEST(wallet_tests, note_data_serialisation) {
    auto sk = libzamerica::SpendingKey::random();
...
TEST(wallet_tests, FindMyNotes) {
    ...
    auto sk = libzamerica::SpendingKey::random();
    auto sk2 = libzamerica::SpendingKey::random();
