...
ZCJoinSplit* pzamericaParams = NULL;
...
void Shutdown()
{
    ...
    pwalletMain = NULL;
#endif
    delete pzamericaParams;
    pzamericaParams = NULL;
    ECC_Stop();
    LogPrintf("%s: done\n", __func__);
}
...
std::string HelpMessage(HelpMessageMode mode)
{
    ...
    strUsage += HelpMessageOpt("-conf=<file>", strprintf(_("Specify configuration file (default: %s)"), "zamerica.conf"));
...
#ifndef WIN32
    strUsage += HelpMessageOpt("-pid=<file>", strprintf(_("Specify pid file (default: %s)"), "zamericad.pid"));
#endif
...
std::string LicenseInfo()
{
    return FormatParagraph(strprintf(_("Copyright (C) 2009-%i The Bitcoin Core Developers"), COPYRIGHT_YEAR)) + "\n" +
           FormatParagraph(strprintf(_("Copyright (C) 2015-%i The ZAmerica Developers"), COPYRIGHT_YEAR)) + "\n" +
           "\n" +
...
static void ZC_LoadParams()
{
    ...
    pzamericaParams = ZCJoinSplit::Unopened();

    LogPrintf("Loading verifying key from %s\n", vk_path.string().c_str());
    gettimeofday(&tv_start, 0);

    pzamericaParams->loadVerifyingKey(vk_path.string());

    gettimeofday(&tv_end, 0);
    elapsed = float(tv_end.tv_sec-tv_start.tv_sec) + (tv_end.tv_usec-tv_start.tv_usec)/float(1000000);
    LogPrintf("Loaded verifying key in %fs seconds.\n", elapsed);

    pzamericaParams->setProvingKeyPath(pk_path.string());
}
...
#define PROCESS_DEP_ENABLE 0x00000001
    ...
    // Sanity check
    if (!InitSanityCheck())
        return InitError(_("Initialization sanity check failed. ZAmerica is shutting down."));
    ...
    try {
        ...
        if (!lock.try_lock())
            return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZAmerica is probably already running."), strDataDir));
    } catch(const boost::interprocess::interprocess_exception& e) {
        return InitError(strprintf(_("Cannot obtain a lock on data directory %s. ZAmerica is probably already running.") + " %s.", strDataDir, e.what()));
    }
    ...
    if (GetBoolArg("-shrinkdebugfile", !fDebug))
        ShrinkDebugFile();
    LogPrintf("\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n\n");
    LogPrintf("ZAmerica version %s (%s)\n", FormatFullVersion(), CLIENT_DATE);
    ...
    // Initialize ZAmerica circuit parameters
    ZC_LoadParams();
