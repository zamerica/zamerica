...
#if defined(NDEBUG)
# error "ZAmerica cannot be compiled without assertions."
#endif
...
const string strMessageMagic = "ZAmerica Signed Message:\n";
...
bool CheckTransaction(const CTransaction& tx, CValidationState &state)
{
    ...
    if (!CheckTransactionWithoutProofVerification(tx, state)) {
        ...
        BOOST_FOREACH(const JSDescription &joinsplit, tx.vjoinsplit) {
            if (!joinsplit.Verify(*pzamericaParams, tx.joinSplitPubKey)) {
...
void ThreadScriptCheck() {
    RenameThread("zamerica-scriptch");
